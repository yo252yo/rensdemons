...
class LayeredImage extends VisualElement {
  constructor(path, w, h, container) {
    ...
    this.layer1 = new StaticSprite(path.replace("$", "a"), 'background', w, h, div); //background
    this.layer2 = new StaticSprite(path.replace("$", "b"), 'obj_light', w, h, div); //void
    this.layer3 = new StaticSprite(path.replace("$", "c"), 'obj_dark', w, h,  div); //obj_light
